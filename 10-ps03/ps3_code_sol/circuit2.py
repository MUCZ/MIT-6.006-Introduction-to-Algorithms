...
class Wire(object):
  ...
  def intersects(self, other_wire):
    ...
    if self.is_horizontal():
      ...
      h = other_wire
      v = self
    if v.y1>h.y1 or h.y1>v.y2 :
      return False
    if h.x1> v.x1 or v.x1>h.x2:
      return False
    return True
...
class RangeIndex(object):
  ...
  def sort(self):
    for i in range(len(self.data)):
      preIndex = i-1
      current = self.data[i]
      while preIndex >= 0 and self.data[preIndex] > current:
        self.data[preIndex+1] = self.data[preIndex]
        preIndex-=1
      self.data[preIndex+1] = current
    return 
...
class KeyWirePair(object):
  ...
  def __init__(self, key, wire,lr = None):
  ...
  def __gt__(self, other):
    ...
    if(self.key > other.key):
      ...
      if(self.lr == 'l' or other.lr == 'r'):
        return False 
      if(self.lr == 'r' or other.lr == 'l'):
        return True 
      else :
        return (self.wire_id > other.wire_id)
  ...
  def __repr__(self):
    ...
    return  '<key: ' + str(self.key) + ' lr: ' + str(self.lr) + '>'
  ...
  def _events_from_layer(self, layer):
    ...
    for wire in layer.wires.values():
      if wire.is_horizontal():
        interest.add(KeyWirePair(wire.x1,wire,'l'))
        interest.add(KeyWirePair(wire.x2,wire,'r'))
      else :
        interest.add(KeyWirePair(wire.x1,wire))

    interest.sort2()

    for w in interest.data:
      if w.wire.is_horizontal() and w.lr=='l':
        self.events.append([w.wire.x1, 0, w.wire.object_id, 'add', w.wire])
        # print("add add")
      elif w.wire.is_horizontal() and w.lr =='r':
        self.events.append([w.wire.x2, 0, w.wire.object_id, 'remove', w.wire])
        # print("add remove")
      else:
        self.events.append([w.wire.x1, 1, w.wire.object_id, 'query', w.wire])
        # print("add query")
